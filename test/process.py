...
def plot_d_v(movie, dst):
    ds = xr.open_dataset(os.path.join(dst, "q_depth.nc"))
    # extract values of v_eff
    v_eff = ds.v_eff.mean(dim="time").values
    z = ds.zcoords.values
    plt.plot(np.maximum(-16.33-z, 0), v_eff, 'b.')
    # for v in v_eff:
    #     print(len(v), len(z))
    #     plt.plot(np.maximum(-16.33-z, 0), v, 'b.')
    plt.show()
