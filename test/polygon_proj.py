...
box = shapely.geometry.box(*shapely.affinity.rotate(polygon_user, -45, origin=ext[2]).bounds)
box_user = shapely.affinity.rotate(box, 45, origin=ext[2])

# plot_coords(ax, polygon.interiors[0])
# plot_coords(ax, polygon.exterior)

patch = PolygonPatch(polygon_user, facecolor='b', edgecolor='b', alpha=0.5,
                     zorder=2)
patch2 = PolygonPatch(polygon, facecolor='k', edgecolor='k', alpha=0.5,
                     zorder=2)
patch3 = PolygonPatch(box_user, facecolor='b', edgecolor='b', alpha=0.5,
                     zorder=2)
patch4 = PolygonPatch(box, facecolor='k', edgecolor='k', alpha=0.5,
                     zorder=2)
ax.add_patch(patch2)
ax.add_patch(patch)
ax.add_patch(patch3)
